...
# Usage: ./pdf-to-mp4-w-gif.sh input.pdf holds.csv
...
if [ $# -ne 2 ]; then
  echo "Usage: $0 input.pdf holds.csv"
  exit 1
fi
...
for hold in "${holds[@]}"; do
  ...
  magick "${base}-$(printf %03d $frame_num).svg" "temp.png"
  for ((j = 1; j <= hold; j++)); do
...
# Create h.264 pencil test video
ffmpeg -framerate 24 -i "png_sequence/frame%05d.png" -vf scale=${width}:-2 -c:v libx264 -pix_fmt yuv420p "${base}.mp4"

# if you want a GIF output just remove the '#'
ffmpeg -framerate 24 -i "png_sequence/frame%05d.png" -vf scale=${width}:-1 "${base}.gif"

# add a "#" to the lines below if you want to keep the outputs
#
rm -r png_sequence
rm "${svgs[@]}"
